...
class DeleteTerpiController extends Controller
{
    ...
    function DeleteTerpiData($KostimiID){
        $isDeleteSuccess = Terpi_tabula::where('KostimiID', $KostimiID)->delete();

        if($isDeleteSuccess) echo '<script>alert("Kostīms tika izdzēsts");</script>';
        else echo '<script>alert("Kostīms netika izdzēsts");</script>';

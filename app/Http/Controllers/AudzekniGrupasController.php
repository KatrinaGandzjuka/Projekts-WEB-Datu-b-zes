...
class AudzekniGrupasController extends Controller
{
    function AudzekniGrupasIndex($GrupasNosaukums){
        $LietotajsData = Lietotajs_tabula::whereIn('LomaID', [1, 3])
            ->where(function($query) use ($GrupasNosaukums) {
                $query->whereExists(function($subquery) use ($GrupasNosaukums) {
                    $subquery->select('*')
                          ->from('audzeknisgrupa')
                          ->whereRaw('AudzeknaGrupasPersonasKods = personasKods')
                          ->where('GrupasAudzeknaNosaukums', $GrupasNosaukums);
                })
                ->orWhereExists(function($subquery) use ($GrupasNosaukums) {
                    $subquery->select('*')
                          ->from('pedagogsgrupa')
        ...
        return view('audzekniGrupas', ['LietotajsData' => $LietotajsData, 'LomasData' => $LomasData, 'GrupasNosaukums' => $GrupasNosaukums, 'GrupasData' => $GrupasData]);
    }
    ...
    function PievAudzAudzekniGrupasIndex($GrupasNosaukums){
        ...
        $LomasData = Lomas_tabula::all();
        $GrupasData = Grupas_tabula::all();
        return view('pievAudzAudzekniGrupas', ['GrupasNosaukums'=>$GrupasNosaukums, 'LietotajsData'=> $LietotajsData, 'LomasData'=> $LomasData]);
    }
    ...
    function AudzekniGrupasDataInsert($GrupasNosaukums, $personasKods){
       ...
       $isAudzekniGrupasInsertSuccess = AudzeknisGrupa_tabula::insert([
        ...
        if($isAudzekniGrupasInsertSuccess) echo '<script>alert("Audzēknis tika pievienots grupai ;)");</script>';
        else echo '<script>alert("Audzēknis netika pievienots grupai :(");</script>';

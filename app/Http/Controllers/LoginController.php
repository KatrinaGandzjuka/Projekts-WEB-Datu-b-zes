...
class LoginController extends Controller
{
    ...
    function LoginPost(Request $request){
        ...
        if($lietotajs){
            session(['personasKods' => $personasKods, 'LomaID' => $lietotajs->LomaID]);
...
function AudzeknisIndex($personasKods){
    ...
    if (count($LietotajsData) > 0) {
        return view('audzeknis', ['ld'=>$LietotajsData[0], 'GrupasData'=>$GrupasData, 'PedagogsData'=>$PedagogsData, 'TerpiData'=>$TerpiData, 'VecaksData'=>$VecaksData]);
    } else {
        return view('audzeknis', ['ld'=>null, 'GrupasData'=>$GrupasData, 'PedagogsData'=>$PedagogsData, 'TerpiData'=>$TerpiData, 'VecaksData'=>$VecaksData]);
    }
}
...
function VecaksIndex($personasKods) {
    ...
    $BernsData = BernsVecaks_tabula::join('lietotajs', 'bernsvecaks.BernaPersonasKods', '=', 'lietotajs.personasKods')
        ->where('bernsvecaks.VecakaPersonasKods', $personasKods)
        ->get(['lietotajs.Vards', 'lietotajs.Uzvards', 'lietotajs.personasKods', 'lietotajs.Talrunis', 'lietotajs.Epasts', 'lietotajs.DzimDiena']);

    if (count($LietotajsData) > 0) {
        return view('vecaks', ['ld'=>$LietotajsData[0], 'BernsData'=>$BernsData]);
    } else {
    ...
    function 
        ...
        if (count($LietotajsData) > 0) {
            return view('pedagogs', ['ld'=>$LietotajsData[0], 'GrupasData'=>$GrupasData, 'AudzeknisData'=>$AudzeknisData, 'TerpiData'=>$TerpiData]);
        } else {
            return view('pedagogs', ['ld'=>null, 'GrupasData'=>$GrupasData, 'AudzeknisData'=>$AudzeknisData, 'TerpiData'=>$TerpiData]);
    }

...
class AudzekniKostimiController extends Controller
{
    function AudzekniKostimiIndex($KostimiID){
        $LietotajsData = Lietotajs_tabula::where('LomaID', 1)
            ...
            ->leftJoin('audzeknisgrupa', 'lietotajs.personasKods', '=', 'audzeknisgrupa.AudzeknaGrupasPersonasKods')
            ->get(['lietotajs.personasKods', 'lietotajs.Vards', 'lietotajs.Uzvards', 'audzeknisgrupa.GrupasAudzeknaNosaukums']);
        $LomasData = Lomas_tabula::all();
        $KostimiData = Terpi_tabula::all();
        return view('audzekniKostimi', ['KostimiID'=>$KostimiID, 'LietotajsData'=> $LietotajsData, 'LomasData'=> $LomasData]);
    }
   ...
   function AudzekniKostimiSavaktIndex($KostimiID){
    ...
    $LomasData = Lomas_tabula::all();
    $KostimiData = Terpi_tabula::all();
    return view('audzekniKostimiSavakt', ['KostimiID'=>$KostimiID, 'LietotajsData'=> $LietotajsData, 'LomasData'=> $LomasData]);
}
...
function AudzekniKostimiGrupaIndex($KostimiID){
    ...
    return view('audzekniKostimiGrupas', ['KostimiID'=>$KostimiID, 'GrupasData'=> $GrupasData, 'audzekniKostimiData' => $audzekniKostimiData]);
}
